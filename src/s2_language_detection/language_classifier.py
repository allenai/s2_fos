...
class LanguageClassifier:
    def __init__(self, data_dir: str = None):
        ...
        if data_dir is None:
            self.model_path = os.path.join(
                PROJECT_ROOT_PATH, os.pardir, "data", FASTTEXT_FNAME
            )
        else:
    ...
    def detect_language(
        self, text_title: str = None, text_abstract: str = None
    ) -> Union[str, float]:
        """
        ...
        if text_abstract and "no abstract" not in text_abstract:
            (
                predicted_language_abstract,
                soft_max_pred_abstract,
            ) = self.detect_language_single_entity(text_abstract)
