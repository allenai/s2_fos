        ...
        for _ in range(MAX_RETRIES):
            try:
                ...
                if not search_for_fos_response(message_content):
                    ...
                    chat_completion = call_openai_api(model, messages, temperature=0)
                open_ai_calls[key] = {"prompt": messages, "result": serialize_chat_completion(chat_completion)}
                break
...
def main():
    ...
    jsonl_file_path = os.path.join(PROJECT_ROOT_PATH, "..", "data", "paper_title_abstract_example_file.json")
    ...
    for idx, (paper_sha, title, abstract, journal_name) in enumerate(meta_data_np):
        ...
        if (idx + 1) % 100 == 0:
            save_openai_calls(
                open_ai_calls, os.path.join(PROJECT_ROOT_PATH, "..", "data", f"open_ai_most_popular_{idx + 1}.json")
            )
    save_openai_calls(open_ai_calls, os.path.join(PROJECT_ROOT_PATH, "..", "data", "open_ai_most_popular_final.json"))
